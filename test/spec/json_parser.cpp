...
static Result parseOneExpected(const simdjson::dom::object &Element) {
  ...
  if (Value.type() == simdjson::dom::element_type::ARRAY) {
    ...
    // Full shape string (e.g. "f32x4"): lane_type + "x" + 16/sizeof(lane).
    size_t LaneSize = 0;
    auto LaneIt = LaneSizeKindMap().find(LaneType);
    auto LSK = (LaneIt != LaneSizeKindMap().end()) ? LaneIt->second
                                                   : LaneSizeKind::Unknown;
    ...
    // Scan lanes for NaN patterns; NaN lanes store 0 in the value.
    ...
    // No NaN lanes: drop the vector to save space.
    if (!HasNaN) {
      R.V128LaneNaN.clear();
    }

    // Build V128 by parsing each lane manually.
    uint128_t V128Val;
    switch (LaneSize) {
    case 8: {
  ...
  // Typed null: keep a concrete RefVariant of the expected heap type
  // (AnyNullRef stays false) so compareResult dispatches per heap type; only
  // the bare untyped "ref" null (handled above) matches any null reference.
  if (ValueStr == "null"sv) {
    switch (*TC) {
    case TypeCode::FuncRef:
    case TypeCode::ExternRef:
  ...
  // Reference types with opaque or non-null values (null was handled above).
  switch (*TC) {
  case TypeCode::ExternRef:
  case TypeCode::AnyRef:
...
// ---------------------------------------------------------------------------
// Parse an "either" array: a flat list of alternatives for one result
// position, returned as a single ResultOrEither.
// ---------------------------------------------------------------------------
...
static Action parseAction(const simdjson::dom::object &ActObj,
  ...
  if (!ActObj["module"].get(ModName)) {
    Act.ModuleName = ownStr(OwnedStrings, ModName);
  }
...
// ---------------------------------------------------------------------------
// Resolve the module_type field into a ModuleType
// ---------------------------------------------------------------------------
...
Expect<JsonScript> parseJson(const std::filesystem::path &JsonPath) {
  ...
  // simdjson implicit conversions and std::stoul/stoull throw on malformed or
  // unexpected input; keep the Expect<> contract by catching and reporting
  // instead of letting the exception escape the test binary.
  try {
    for (const simdjson::dom::object &Cmd : CmdArray) {
      ...
      case CommandType::AssertTrap: {
        // wast2json emits assert_trap either with an "action" (invoke/get trap)
        // or with a module reference (instantiation trap). Handle both.
        simdjson::dom::object ActObj;
        if (!Cmd["action"].get(ActObj)) {
          SC.Act = parseAction(ActObj, Script.OwnedStrings);
        } else {
          std::string_view FileName;
          if (!Cmd["filename"].get(FileName)) {
            SC.ModuleSource = ownStr(Script.OwnedStrings, FileName);
            SC.ModType = resolveModType(Cmd);
          }
        }
    ...
    spdlog::error("Failed to parse JSON commands in {}: {}"sv,
